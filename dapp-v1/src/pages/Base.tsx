...
export const Base = () => {
  ...
  const chainIdToUse = account.chainId !== undefined ? account.chainId : chainId;
  const graphUrl = getProtocolGraphUrl(chainIdToUse);
  let client: Client | null = null;

  // If the graphUrl is empty, and account is connected then the network is not supported
  if (graphUrl === "" && account.chainId !== undefined) {
    openSwitchNetworks(); // Open the switch network modal
  }
  // If the graphUrl is not empty, then use it to create the client
  else if (graphUrl !== "") {
    client = new Client({
      url: graphUrl,
      ...
      <Provider value={client || {}}>
        ...
        >
          <div className="offcanvas-header">
            <NavLink to="/" className="sidebar-brand d-flex align-items-center me-auto">
              <img
        ...
        <div className="content-wrapper min-vh-100">
                    ...
                    return (
                      <>
                        <div className="d-flex">
                          ...
                          >
                            {isConnected && (
                              <small>{ensName || truncatedAddress}</small>
                            )}
                            {!isConnected && <small>Connect</small>}
                            <i className="fa-solid fa-wallet ms-2" />
                          </button>
          ...
          {/* Content start */}
          <main className="mt-4 mt-xl-5">{client ? <Outlet /> : null}</main>
          {/* Content end */}
        </div>
