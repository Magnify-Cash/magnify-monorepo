...
import "./interfaces/IMagnifyCashV1.sol";
import "./interfaces/IMagnifyERC721V1.sol";

contract MagnifyCashV1 is
    IMagnifyCashV1,
    Ownable,
    Pausable,
    ERC721Holder,
    ERC1155Holder
{
    ...
    function initializeNewLendingDesk(
        ...
        IMagnifyERC721V1(lendingKeys).mint(msg.sender, lendingDeskId);
    ...
    function setLendingDeskLoanConfigs(
        ...
        if (IMagnifyERC721V1(lendingKeys).ownerOf(_lendingDeskId) != msg.sender)
    ...
    function initializeNewLoan(
        ...
        */
        ...
        IMagnifyERC721V1(obligationNotes).mint(msg.sender, loanId);
    ...
    function makeLoanPayment(
        ...
        address borrower = IMagnifyERC721V1(obligationNotes).ownerOf(_loanId);
    ...
    function liquidateDefaultedLoan(uint256 _loanId) external {
        ...
        if (loan.status != LoanStatus.Active) revert LoanIsNotActive();
        if (
            IMagnifyERC721V1(lendingKeys).ownerOf(loan.lendingDeskId) != msg.sender
