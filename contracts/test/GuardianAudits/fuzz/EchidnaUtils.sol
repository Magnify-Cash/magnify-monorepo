...
contract EchidnaUtils {
    ...
    function deal(address token, address user, uint256 amount) internal {
        try ERC20Mock(token).mint(user, amount) {}
        catch {
            ERC20Mock(token).transfer(user, amount);
        }
    }
    ...
    function assertEq(bool a, bool b, string memory reason) internal {
        if (a != b) {
            bytes memory assertMsg = abi.encodePacked("Invalid: ", a, "!=", b, ", reason: ", reason);
            emit AssertEqFail(string(assertMsg));
            assert(false);
        }
    }
    ...
    /// @notice int256 version of assertNotEq
    ...
    /// @notice address version of assertNotEq
    ...
    /// @notice Bounds value to be between low and high, both inclusive
    function bound(uint256 value, uint256 low, uint256 high) internal returns (uint256) {
    ...
    /// @notice Bounds value to be between low and high, both inclusive, no logs
    function _bound(uint256 value, uint256 low, uint256 high) internal pure returns (uint256) {
        if (value < low || value > high) {
    ...
    /// @notice int256 version of bound
    function bound(int256 value, int256 low, int256 high) internal returns (int256) {
        if (value < low || value > high) {
